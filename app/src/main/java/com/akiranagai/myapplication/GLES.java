...
public class GLES {
    ...
    //点　（Point Sprite）
    //頂点シェーダのコード
    private final static String PointSprite_VSCODE =
            "precision mediump float;" +
    ...
    //照明ありのときの描画
    //頂点シェーダのコード
    private final static String ObjectWithLight2_VSCODE =
            //ZBuffer値を対数拡張するための定数
            "uniform float u_AA;"+
                    "uniform float u_BB;"+
                    "uniform float u_CC;"+
                    ...
                    "uniform vec4 u_LightPos2;" +     //光源の位置（カメラビュー座標系）2
                    //マテリアル
                    "uniform vec4 u_MaterialAmbient;" +   //マテリアルの環境光色
                    "uniform vec4 u_MaterialDiffuse;" +   //マテリアルの拡散光色
                    "uniform vec4 u_MaterialSpecular;" +  //マテリアルの鏡面光色
                    ...
                    "vec3 P=vec3(u_MMatrix*a_Position);" +
                    "vec3 L=normalize(vec3(u_LightPos)-P);" +  //光源方向単位ベクトル
                    "vec3 L2=normalize(vec3(u_LightPos2)-P);" +  //光源方向単位ベクトル2
                    "vec3 N=normalize(mat3(u_MMatrix)*a_Normal);" +  //法線単位ベクトル
                    "float dotLN=max(dot(L,N),0.0);" +
                    "float dotLN2=max(dot(L2,N),0.0);" +
                    "vec4 diffuseP=vec4(dotLN);" +
                    "vec4 diffuseP2=vec4(dotLN2);" +
                    "vec4 diffuse=diffuseP*u_LightDiffuse*u_MaterialDiffuse;" +
                    "vec4 diffuse2=diffuseP2*u_LightDiffuse*u_MaterialDiffuse;" +
                    //鏡面光の計算
                    "vec3 V=normalize(-P);" +  //視点方向単位ベクトル
                    "float dotNLEffect=ceil(dotLN);" +
                    "float dotNLEffect2=ceil(dotLN2);" +
                    "vec3 R=2.*dotLN*N-L;" +
                    "vec3 R2=2.*dotLN2*N-L2;" +
                    "float specularP=pow(max(dot(R,V),0.0),u_MaterialShininess)*dotNLEffect;" +
                    "float specularP2=pow(max(dot(R,V),0.0),u_MaterialShininess)*dotNLEffect2;" +
                    "vec4 specular=specularP*u_LightSpecular*u_MaterialSpecular;" +
                    "vec4 specular2=specularP2*u_LightSpecular*u_MaterialSpecular;" +
                    //色の指定
                    "v_Color=ambient+diffuse+diffuse2+specular+specular2;" +
                    //位置の指定
                    "gl_Position=u_PMMatrix*a_Position;" +
                    //ZBuffer拡張用の拡張計算
                    //p=log(z + c) を計算
    ...
    //フラグメントシェーダのコード
    private final static String ObjectWithLight2_FSCODE =
            "precision mediump float;" +
                    "varying vec4 v_Color;" +
                    "void main(){" +
                    "gl_FragColor=v_Color;" +
...
/*
    ...
    //照明なしのときのテクスチャ描画
    //頂点シェーダのコード
    private final static String SimpleTextureSimpleZ_VSCODE =
            //ZBuffer値を対数拡張するための定数
            "uniform float u_AA;"+
                    "uniform float u_BB;"+
                    "uniform float u_CC;"+
    ...
    //フラグメントシェーダのコード
    private final static String SimpleTextureSimpleZ_FSCODE =
            "precision mediump float;" +
                    "uniform sampler2D u_Texture;" +
                    "varying vec2 v_Texcoord;" +
                    "void main() {" +
...
*/
    ...
    public static int lightAmbient2Handle; //光源の環境光色ハンドル2
    public static int lightDiffuseHandle; //光源の拡散光色ハンドル
    public static int lightDiffuse2Handle; //光源の拡散光色ハンドル2
    public static int lightSpecularHandle;//光源の鏡面光色ハンドル
    public static int lightSpecular2Handle;//光源の鏡面光色ハンドル2
    public static int lightPosHandle;     //光源の位置ハンドル
    public static int lightPos2Handle;     //光源の位置ハンドル2
    ...
    private static float[] LightPos2 = new float[4];    //光源の座標　x,y,z　（ワールド座標）2
    private static float[] CVLightPos2 = new float[4];    //光源の座標　x,y,z　（カメラビュー座標）2
    ...
    private static float[] LightAmb2 = new float[4];    //光源の環境光2
    private static float[] LightDif2 = new float[4];    //光源の乱反射光2
    private static float[] LightSpc2 = new float[4];    //光源の鏡面反射反射光2
    ...
    public static boolean makeProgram() {
        ...
        programNumber.put("SP_TextureWithLight_Obstacle", SP_TextureWithLight_Obstacle);
        ...
        programNumber.put("SP_SimpleObjectVTXColor", SP_SimpleObjectVTXColor);
        if (SP_SimpleObjectVTXColor == INVALID) return false;
        SP_PointBlur = makeProgram0(PointBlur_VSCODE, PointBlur_FSCODE);
        programNumber.put("SP_PointBlur", SP_PointBlur);
        if (SP_PointBlur == INVALID) return false;
        SP_PointSprite = makeProgram0(PointSprite_VSCODE, PointSprite_FSCODE);
        programNumber.put("SP_PointSprite", SP_PointSprite);
        if (SP_PointSprite == INVALID) return false;
        SP_ObjectWithLight2 = makeProgram0(ObjectWithLight2_VSCODE, ObjectWithLight2_FSCODE);
        programNumber.put("SP_ObjectWithLight2", SP_ObjectWithLight2);
        if (SP_ObjectWithLight2 == INVALID) return false;
    ...
    public static void putLightAttribute2(float[] amb, float[] dif, float[] spc) {
        System.arraycopy(amb, 0, LightAmb2, 0, 4);
        System.arraycopy(dif, 0, LightDif2, 0, 4);
        System.arraycopy(spc, 0, LightSpc2, 0, 4);
    }
    ...
    public static void selectProgram(int programID) {
        ...
        if (programID == SP_ObjectWithLight) {
            ...
            useLighting = true;
        }else if (programID == SP_ObjectWithLight2) {
                GLES20.glUseProgram(programID);
                //光源のハンドルの取得
                lightAmbientHandle = GLES20.glGetUniformLocation(programID, "u_LightAmbient");
                lightAmbient2Handle = GLES20.glGetUniformLocation(programID, "u_LightAmbient2");
                lightDiffuseHandle = GLES20.glGetUniformLocation(programID, "u_LightDiffuse");
                lightDiffuse2Handle = GLES20.glGetUniformLocation(programID, "u_LightDiffuse2");
                lightSpecularHandle = GLES20.glGetUniformLocation(programID, "u_LightSpecular");
                lightSpecular2Handle = GLES20.glGetUniformLocation(programID, "u_LightSpecular2");
                lightPosHandle = GLES20.glGetUniformLocation(programID, "u_LightPos");
                lightPos2Handle = GLES20.glGetUniformLocation(programID, "u_LightPos2");
                //マテリアルのハンドルの取得
                materialAmbientHandle = GLES20.glGetUniformLocation(programID, "u_MaterialAmbient");
                materialDiffuseHandle = GLES20.glGetUniformLocation(programID, "u_MaterialDiffuse");
                materialSpecularHandle = GLES20.glGetUniformLocation(programID, "u_MaterialSpecular");
                ...
                //光源位置の指定   (x, y, z, 1)
                GLES20.glUniform4f(GLES.lightPosHandle,CVLightPos[0], CVLightPos[1], CVLightPos[2], 1.0f);
                GLES20.glUniform4f(GLES.lightPos2Handle,CVLightPos2[0], CVLightPos2[1], CVLightPos2[2], 1.0f);
                //光源色の指定 (r, g, b,a)
                GLES20.glUniform4f(GLES.lightAmbientHandle, LightAmb[0], LightAmb[1], LightAmb[2], LightAmb[3]); //周辺光
                GLES20.glUniform4f(GLES.lightDiffuseHandle, LightDif[0], LightDif[1], LightDif[2], LightDif[3]); //乱反射光
                GLES20.glUniform4f(GLES.lightSpecularHandle, LightSpc[0], LightSpc[1], LightSpc[2], LightSpc[3]); //鏡面反射光
            GLES20.glUniform4f(GLES.lightAmbient2Handle, LightAmb2[0], LightAmb2[1], LightAmb2[2], LightAmb2[3]); //周辺光2
            GLES20.glUniform4f(GLES.lightDiffuse2Handle, LightDif2[0], LightDif2[1], LightDif2[2], LightDif2[3]); //乱反射光2
            GLES20.glUniform4f(GLES.lightSpecular2Handle, LightSpc2[0], LightSpc2[1], LightSpc2[2], LightSpc2[3]); //鏡面反射光2
                useLighting = true;
            ...
            GLES20.glUniform4f(GLES.lightSpecularHandle, LightSpc[0], LightSpc[1], LightSpc[2], LightSpc[3]); //鏡面反射光
            useLighting = true;
        } else if (programID == SP_SimpleTexture || programID == SP_SimpleTextureSimpleZ) {
            GLES20.glUseProgram(programID);
            //テクスチャのハンドルの取得
            texcoordHandle = GLES20.glGetAttribLocation(programID, "a_Texcoord");
            textureHandle = GLES20.glGetUniformLocation(programID, "u_Texture");
            ...
            GLES20.glEnableVertexAttribArray(texcoordHandle);
            useLighting = false;
        } else if (programID == SP_PointBlur || programID == SP_PointSprite) {
            GLES20.glUseProgram(programID);
            //光源を使わない時のマテリアルの色のハンドルの取得
    ...
    public static void deselectProgram() {
        ...
        } else if (currentProgram == SP_ObjectWithLight2) {
            GLES20.glDisableVertexAttribArray(normalHandle);
        } else if (currentProgram == SP_SimpleTexture) {
            GLES20.glDisableVertexAttribArray(texcoordHandle);
        } else if (currentProgram == SP_SimpleTextureSimpleZ) {
            GLES20.glDisableVertexAttribArray(texcoordHandle);
    ...
    public static void setLightPosition2(float[] lp) {
        System.arraycopy(lp, 0, LightPos2, 0, 4);
    }
