...
public class GestureDetector {
    ...
    private final long TimeConstant=600;  //シングルショートタップ時間　[msec]
    private final long TimeConstant1A=50;  //切り替え不感時間　[msec]
    private final long TimeConstant1B=100;  //切り替え不感時間　[msec]
    private final long TimeConstant2=1400;  //最短シングルロングタップ時間　[msec]
    private final long TimeConstant3=4000;  //最長シングルロングタップ時間　[msec]
    //private final long TimeLongDown=2000;  //長押し時間
    private final float DisplacementLimit=1500f;  //タップ時の移動許容定義(距離の２乗)
    private ScreenInput sInput;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (ActionMasked) {
            case MotionEvent.ACTION_DOWN://最初のタッチ
                Log.d("message", "ACTION_DOWN: " + ActionMasked);
            ...
            case MotionEvent.ACTION_POINTER_UP://２つ目以降のタッチの終了
                ...
                if (found!=-1) {
                    for (; i < nValidPointers - 1; i++) mPointerID[i] = mPointerID[i + 1];  //要素を1つづつずらす
                    mPointerID[i] = -1;  //最後の要素を無し(-1)に設定
                    nValidPointers--;  //要素数 -1する
                    range = 3;
                ...
                if (nValidPointers==1) {
                    if (mEndTime!=mBeginTime) {
                        ...
                        if (deltat<TimeConstant){
                            if(d2<DisplacementLimit) {
                                mListener.onShortTap(this);
                            }else{
                                mListener.onFling(this);
                            }
